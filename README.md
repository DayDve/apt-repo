...
[![Apps](https://img.shields.io/badge/apps-25-blue)](https://apt.smbit.pro/packages)
...
```bash
sudo curl -fsSL https://apt-repo.daydve.workers.dev/apt-key.asc \
  -o /etc/apt/keyrings/daydve-apt-repo.asc && \
echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/daydve-apt-repo.asc] \
  https://apt-repo.daydve.workers.dev noble main" \
